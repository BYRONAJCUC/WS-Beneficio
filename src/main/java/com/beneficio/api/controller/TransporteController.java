...
public class TransporteController {
    ...
    public ResponseEntity<Transporte> getById(@PathVariable("placa") String placa){
        if(!transporteService.existsByPlaca(placa))
            return new ResponseEntity(new Mensaje("no existe"), HttpStatus.NOT_FOUND);
        Transporte transporte = transporteService.getByPlaca(placa).get();
        return new ResponseEntity(transporte, HttpStatus.OK);
    }
...
//            return new ResponseEntity(new Mensaje("ese dpi ya existe"), HttpStatus.BAD_REQUEST);
        Transporte transporte = new Transporte(transporteDto.getPlaca(), transporteDto.getMarca(), transporteDto.getModelo(),transporteDto.getColor(),transporteDto.getPeso(), transporteDto.getUsuarioCreacion());
